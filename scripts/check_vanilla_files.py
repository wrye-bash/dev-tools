...
"""This module tests a game definition against all files in the target directory

   To use, place this script in the Mopy folder, edit the script to import
   the desired GameInfo class, and the test path to the desired location.
...
game_info_name = u'WSOblivionGameInfo'
# Should be an absolute path
game_path_name = ur'C:\Program Files\ModifiableWindowsApps\Oblivion GOTY (PC)\Oblivion GOTY Spanish'
...
def import_game_type(game_info_name):
    ...
    for importer, modname, ispkg in pkgutil.iter_modules(game_init.__path__):
        ...
        try:
            module = __import__(u'bash.game',globals(),locals(),[modname],-1)
...
def main():
    ...
    if not GAME_TYPE:
        print(u'No game module found with %s' % game_info_name)
        return
    ...
    print(u'Files present in %s that are not in vanilla_files:' % test_path)
    new_files = all_files - vanilla_files
    new_files = {x.s for x in new_files}    # For better console output
    pprint(new_files)
    print()
    print(u'Files that are in vanilla_files that are not in %s' % test_path)
